...
| Where is the approved inbound-call worksheet? | `inbound-client-intake/index.html` plus `agent-suite-intake-continuity.js` | Treat it as the operator-approved 15-step inbound call workflow. On `codex/inbound-intake-live-integration` it uses Agent Suite auth and `/api/clients`; `01_website/experiments/Client-Intake-Guided.html` remains a compatibility redirect. Verify an approved deployment before calling that integration live. |
...
| Inbound client intake sheet | Evermore operator + Agent Suite backend | `inbound-client-intake/index.html` and `agent-suite-intake-continuity.js` (approved call-flow asset; server-first integration is implemented and locally verified on `codex/inbound-intake-live-integration`, but not deployed) |
