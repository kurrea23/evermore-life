...
# Deploy Sarah_Evermore_AI.html to Cloudflare Workers without hardcoding secrets.
# Required env vars:
#   CLOUDFLARE_ACCOUNT_ID
#   CLOUDFLARE_API_TOKEN
# Optional:
#   WORKER_NAME=sarah-evermore

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
HTML_FILE="${1:-$PROJECT_ROOT/01_website/experiments/Sarah_Evermore_AI.html}"
WORKER_NAME="${WORKER_NAME:-sarah-evermore}"
ACCOUNT_ID="${CLOUDFLARE_ACCOUNT_ID:-${ACCOUNT_ID:-}}"
API_TOKEN="${CLOUDFLARE_API_TOKEN:-${CF_API_TOKEN:-}}"

if [[ ! -f "$HTML_FILE" ]]; then
  echo "Sarah HTML file not found: $HTML_FILE" >&2
  exit 1
fi
...
if [[ "$SUCCESS" == "true" ]]; then
  echo "Sarah deployed successfully."
